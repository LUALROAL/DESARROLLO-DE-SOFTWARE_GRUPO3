...
public class ClienteController {
	...
	@PreAuthorize("hasRole('ADMIN')")
	@PostMapping("/guardar")
	public ModelAndView crear(@RequestParam String cedula, @RequestParam String nombre,
			@RequestParam String direccion, @RequestParam String telefono, @RequestParam String correoElectronico) {
		ModelAndView mv = new ModelAndView();
	...
	public ModelAndView actualizar(@RequestParam int id, @RequestParam String cedula,
		...
		if (clienteService.existsByNombre(nombre)
				&& clienteService.getByNombre(nombre).get().getId() != id) {
			mv.setViewName("cliente/editar");
			mv.addObject("error", "ese nombre ya existe");
			mv.addObject("cliente", cliente);
			return mv;
